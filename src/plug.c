...
MUSIALIZER_PLUG void plug_free_resource(void *data) { (void)data; }
...
MUSIALIZER_PLUG void plug_free_resource(void *data) { UnloadFileData(data); }
...
#define PT_GET(pt, index)                                                      \
  (assert(index < (pt)->count),                                                \
   &(pt)->items[((pt)->begin + index) % POPUP_TRAY_CAPACITY])
...
#ifdef MUSIALIZER_MICROPHONE
  if (p->capturing) {
    // TODO: according to documentation drwav_write_pcm_frames may not write all
    // the frames. Make sure it does.
...
static void snap_segment_inside_other_segment(float ls, float rs, float *lt,
                                              float *rt) {
  float dt = *rt - *lt;
  if (rs < *lt || rs < *rt) {
    *rt = rs;
...
static void begin_tooltip_frame(void) { p->tooltip_show = false; }
...
static void end_tooltip_frame(void) {
  ...
  Vector2 text_size =
      MeasureTextEx(p->font, p->tooltip_buffer, fontSize, spacing);
  ...
  align_to_side_of_rect(p->tooltip_element_boundary, &tooltip_boundary,
                        p->tooltip_align);
  ...
  DrawTextEx(p->font, p->tooltip_buffer, position, fontSize, spacing,
             COLOR_TOOLTIP_FOREGROUND);
}

static void tooltip(Rectangle boundary, const char *text, Side align,
                    bool persists) {
  if (!(CheckCollisionPointRec(GetMousePosition(), boundary) || persists))
    return;
  p->tooltip_show = true;
...
static void timeline(Rectangle timeline_boundary, Track *track) {
  ...
  Vector2 startPos = {.x = x, .y = timeline_boundary.y};
  Vector2 endPos = {.x = x,
...
static int button_with_location(const char *file, int line,
                                Rectangle boundary) {
...
// NOTE: This is literally DrawTextEx() copy-pasted from Raylib itself but with
// the max_width support and without newlines
void track_label(Font font, const char *text, Vector2 position, float fontSize,
                 Color tint) {
...
static void tracks_panel_with_location(const char *file, int line,
  ...
  for (size_t i = 0; i < p->tracks.count; ++i) {
    ...
    if (((int)i != p->current_track)) {
      ...
      int state = button_with_id(
          item_id, GetCollisionRec(panel_boundary, item_boundary));
    ...
    // That triggers an old raylib bug with circles tho, so we will have to look
    // into that
    ...
    uint64_t item_id = djb2(id, &i, sizeof(i));
    int state =
        button_with_id(item_id, GetCollisionRec(panel_boundary, item_boundary));

    if ((size.x > max_width)) { // <-- Item needs ScissorMode
      BeginScissorMode(position.x, position.y, max_width, item_boundary.height);

      if (state & BS_HOVEROVER) { // <-- Current item is being hovered on and
                                  // needs scrolling
        static float dt = 0;
        ...
        dt += GetFrameTime();
        if (item_id != hovered_label_id) { // <-- But it is not same as the last
                                           // hovered item, so reset the shift
          px_shift = 0;
          scroll_left = true;
          ...
          if (dt > TRACKLABEL_SCROLL_SECS) {
            dt = 0.0f;
            if ((abs(px_shift) >= size.x - max_width + 10) ||
                (px_shift == 10)) {       // <-- End of scroll (with 10 padding)
    ...
    if (scrolling) {
      ...
      if (CheckCollisionPointRec(mouse, scroll_bar_boundary)) {
        ...
        if (IsMouseButtonReleased(MOUSE_LEFT_BUTTON)) {
          ...
          } else if (scroll_bar_boundary.y + scroll_bar_boundary.height <
                     mouse.y) {
            panel_velocity += -item_size * 16;
          }
        }
...
static int fullscreen_button_with_loc(const char *file, int line,
...
static bool volume_slider_with_location(const char *file, int line,
  ...
  if (expanded) {
    ...
    tooltip(slider_boundary,
            TextFormat("Volume %d%%", (int)floorf(volume * 100.0f)), SIDE_TOP,
            dragging);
  }
  ...
  if (IsKeyPressed(KEY_TOGGLE_MUTE) || (volume_icon_state & BS_CLICKED)) {
    if (volume > 0) {
      saved_volume = volume;
      volume = 0;
    } else {
...
static void popup_tray(Popup_Tray *pt, Rectangle preview_boundary) {
  ...
  for (size_t i = 0; i < pt->count; ++i) {
    ...
    Rectangle popup_boundary = {
        .x = preview_boundary.x + preview_boundary.width - popup_width -
             popup_padding,
        .y = preview_boundary.y + preview_boundary.height -
             (i + 1 - q) * (popup_height + popup_padding),
        .width = popup_width,
        .height = popup_height,
    };
    DrawRectangleRounded(popup_boundary, 0.3, 20,
                         ColorAlpha(COLOR_POPUP_BACKGROUND, alpha));
...
static int cancel_rendering_button_with_location(const char *file, int line,
                                                 Rectangle boundary) {
  uint64_t id = DJB2_INIT;
  id = djb2(id, file, strlen(file));
  id = djb2(id, &line, sizeof(line));
  ...
  Color color = (state & BS_HOVEROVER) ? COLOR_TRACK_BUTTON_HOVEROVER
                                       : COLOR_TRACK_BUTTON_BACKGROUND;
...
static int play_button_with_location(const char *file, int line, Track *track,
                                     Rectangle boundary) {
  uint64_t id = DJB2_INIT;
  id = djb2(id, file, strlen(file));
  id = djb2(id, &line, sizeof(line));
  ...
  float icon_size = 512;
  float scale = HUD_BUTTON_SIZE / icon_size * HUD_ICON_SCALE;
  Rectangle dest = {boundary.x + boundary.width / 2 - icon_size * scale / 2,
...
static int render_button_with_location(const char *file, int line,
                                       Rectangle boundary) {
  uint64_t id = DJB2_INIT;
  id = djb2(id, file, strlen(file));
  id = djb2(id, &line, sizeof(line));
...
static int microphone_button_with_location(const char *file, int line,
                                           Rectangle boundary) {
  uint64_t id = DJB2_INIT;
  id = djb2(id, file, strlen(file));
  id = djb2(id, &line, sizeof(line));
...
static void start_rendering_track(Track *track) {
  ...
  char *output_path = tinyfd_saveFileDialog("Path to rendered video", "./",
                                            NOB_ARRAY_LEN(filter_params),
                                            filter_params, "mp4 video file");
  if (output_path == NULL)
    return;
  ...
  p->ffmpeg = ffmpeg_start_rendering(output_path, p->screen.texture.width,
                                     p->screen.texture.height, RENDER_FPS,
                                     track->file_path);
  SetTargetFPS(0);
  p->rendering = true;
...
static void start_capture(void) {
  ...
  if (!drwav_init_file_write(&p->wav, recording_file_path, &format, NULL)) {
    TraceLog(LOG_ERROR, "DRWAVE: Failed to initialize output file %s",
             recording_file_path);
    return;
  }
  ...
  if (result != MA_SUCCESS) {
    TraceLog(LOG_ERROR, "MINIAUDIO: Failed to initialize capture device: %s",
...
static bool toolbar(Track *track, Rectangle boundary) {
  ...
  if (boundary.width < HUD_BUTTON_SIZE * buttons_count)
    return interacted;
...
static void preview_screen(void) {
  ...
  if (track) { // The music is loaded and ready
    ...
    if (p->fullscreen) {
      // TODO: make timeline somehow visible in fullscreen mode (maybe
      // miniversion of it on the toolbar)
    ...
    if (button(((Rectangle){0, 0, w, h})) & BS_CLICKED) {
      ...
      char *input_path = tinyfd_openFileDialog(
          "Path to music file", "./", NOB_ARRAY_LEN(filter_params),
          filter_params, "music file", allow_multiple_selects);
      if (input_path) {
...
static void capture_screen(void) {
  ...
  if (p->microphone_working) {
    ...
    size_t m = fft_analyze(GetFrameTime());
    fft_render(CLITERAL(Rectangle){0, 0, GetScreenWidth(), GetScreenHeight()},
               m);
  } else {
    ...
    position.x = w / 2 - size.x / 2, position.y = h / 2 - size.y / 2 + fontSize,
    DrawTextEx(p->font, label, position, fontSize, 0, color);
  }
}
#endif // MUSIALIZER_MICROPHONE
...
static void rendering_screen(void) {
  ...
  if (p->ffmpeg == NULL) { // Starting FFmpeg process has failed for some reason
    ...
    if (p->wave_cursor >= p->wave.frameCount &&
        fft_settled()) { // Rendering is finished
      if (!ffmpeg_end_rendering(p->ffmpeg, false)) {
        // NOTE: Ending FFmpeg process has failed, let's mark ffmpeg handle as
        // NULL which will be interpreted as "FFmpeg Failure" on the next frame.
        //
        // It should be safe to set ffmpeg to NULL even if
        // ffmpeg_end_rendering() failed cause it should deallocate all the
        // resources even in case of a failure.
        p->ffmpeg = NULL;
      } else {
        finish_rendering_track(track);
      }
    } else if (IsKeyPressed(KEY_ESCAPE) ||
               p->cancel_rendering) { // Rendering is cancelled
      ...
      if (!ffmpeg_send_frame_flipped(p->ffmpeg, image.data, image.width,
                                     image.height)) {
        // NOTE: we don't check the result of ffmpeg_end_rendering here because
        // we don't care at this point: writing a frame failed, so something
        // went completely wrong. So let's just show to the user the "FFmpeg
        // Failure" screen. ffmpeg_end_rendering should log any additional
        // errors anyway.
...
static void load_assets(void) {
  ...
  p->font = LoadFontFromMemory(GetFileExtension(alegreya_path), data, data_size,
                               FONT_SIZE, NULL, 0);
  ...
  //
  // If the shader can not be compiled maybe we could fallback to software
  // rendering of the texture of a fuzzy circle? The shader does not really do
  // anything particularly special.
  data = plug_load_resource(
  ...
  for (UI_Icon icon = 0; icon < COUNT_UI_ICONS; ++icon) {
    ...
    Image image = LoadImageFromMemory(GetFileExtension(icon_file_paths[icon]),
                                      data, data_size);
...
MUSIALIZER_PLUG void plug_init(FilePathList init_tracks) {
  p = malloc(sizeof(*p));
  ...
  for (size_t i = 0; i < init_tracks.count; ++i) {
    Music music = LoadMusicStream(init_tracks.paths[i]);
    if (IsMusicValid(music)) {
      AttachAudioStreamProcessor(music.stream, callback);
      char *file_path = strdup(init_tracks.paths[i]);
      assert(file_path != NULL);
      nob_da_append(&p->tracks, (CLITERAL(Track){
                                    .file_path = file_path,
                                    .music = music,
...
// We do have fullscreen button, but apparently there is a demand on a
// fullscreen fullscreen mode. Raylib does have ToggleFullscreen(). Let's see
// how we can integrate it into the current UI/UX
